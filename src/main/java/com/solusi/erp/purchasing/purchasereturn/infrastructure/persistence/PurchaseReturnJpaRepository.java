...
public interface PurchaseReturnJpaRepository extends JpaRepository<PurchaseReturnEntity, Long> {
    ...
    @Query("SELECT DISTINCT p FROM PurchaseReturnEntity p LEFT JOIN FETCH p.lines WHERE p.id = :id")
    Optional<PurchaseReturnEntity> findByIdWithLines(@Param("id") Long id);

    @Query(value = "SELECT DISTINCT p FROM PurchaseReturnEntity p LEFT JOIN FETCH p.lines WHERE "
    ...
    @Query(value = "SELECT DISTINCT p FROM PurchaseReturnEntity p LEFT JOIN FETCH p.lines WHERE p.status = :status",
    ...
    @Query(value = "SELECT DISTINCT p FROM PurchaseReturnEntity p LEFT JOIN FETCH p.lines",
    ...
    Optional<PurchaseReturnEntity> findByGeneratedGoodsIssueId(Long generatedGoodsIssueId);
}
