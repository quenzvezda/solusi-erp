package com.solusi.erp.master.partyroletype.web.template;
...
public class PartyRoleTypeTemplateTest {

    @Test
    public void templateReferencesValidProperties() throws Exception {
        InputStream is = getClass().getClassLoader()
                .getResourceAsStream("templates/master/party-role-types/list.html");
        ...
        while (m.find()) {
            ...
            for (java.lang.reflect.Field f : PartyRoleTypeSummaryResponse.class.getDeclaredFields()) {
            ...
            for (java.lang.reflect.Method method : PartyRoleTypeSummaryResponse.class.getMethods()) {
            ...
            assertThat(hasField || hasGetter)
                    .withFailMessage("Property '%s' referenced in template but not present on %s",
                            prop, PartyRoleTypeSummaryResponse.class.getName())
                    .isTrue();
        }
    }
    ...
    public void templateContainsExpectedPlaceholders() throws Exception {
        ...
        assertThat(template).contains("party-role-type-table-container");
