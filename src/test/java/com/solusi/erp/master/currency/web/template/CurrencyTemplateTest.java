package com.solusi.erp.master.currency.web.template;
...
public class CurrencyTemplateTest {

    @Test
    public void templateReferencesValidProperties() throws Exception {
        InputStream is = getClass().getClassLoader().getResourceAsStream("templates/master/currency/list.html");
        ...
        while (m.find()) {
            ...
            for (java.lang.reflect.Field f : CurrencySummaryResponse.class.getDeclaredFields()) {
            ...
            for (java.lang.reflect.Method method : CurrencySummaryResponse.class.getMethods()) {
            ...
            assertThat(hasField || hasGetter)
                    .withFailMessage("Property '%s' referenced in template but not present on %s",
                            prop, CurrencySummaryResponse.class.getName())
                    .isTrue();
        }
    }
    ...
    public void templateContainsExpectedPlaceholders() throws Exception {
        ...
        assertThat(template).contains("currency-table-container");
        assertThat(template).contains("${item.name}");
        assertThat(template).contains("${item.alias}");
    }
}
