...
class UpdateTaxUseCaseTest {
    ...
    void execute_updatesMutableFields() {
        ...
        Tax existing = new Tax(metadata, "TX-01", "Old Name", BigDecimal.ONE, "Old Note", false, true);

        when(repository.findById(1L)).thenReturn(Optional.of(existing));
        when(repository.save(any(Tax.class))).thenAnswer(invocation -> invocation.getArgument(0));

        Tax result = useCase.execute(1L, "New Name", BigDecimal.TEN, "New Note", true, false);
    ...
    void execute_throwsDomainException_whenNotFound() {
        ...
        assertThrows(DomainException.class,
                () -> useCase.execute(99L, "Name", BigDecimal.ONE, null, false, true));
    }
}
