...
class FindGoodsReceiptsUseCaseTest {
    ...
    void execute_delegatesToRepository() {
        ...
        when(goodsReceiptRepository.findAll("PO-0007", pageable)).thenReturn(expected);

        Page<GoodsReceipt> result = useCase.execute("PO-0007", pageable);

        assertThat(result).isSameAs(expected);
        verify(goodsReceiptRepository).findAll("PO-0007", pageable);
    }
}
